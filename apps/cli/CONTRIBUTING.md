...
- **New scanners** — Cursor, Windsurf, Codex, Aider, Continue, anything else with a local session archive. Drop a new file under [`src/scanners/`](./src/scanners/), wire it up to the upload payload. The `source` enum in [`packages/core/src/redaction.ts`](../../packages/core/src/redaction.ts) needs to grow to match.
