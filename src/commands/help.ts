...
export function runHelp(): void {
  ...
  process.stdout.write(
    [
      ...
      "",
      `    npx vibeking installations reset ${c.dim("wipe local installationId — fresh id on next publish")}`,
      `    npx vibeking inspect-upload      ${c.dim("print the exact payload that would be uploaded")}`,
      `    npx vibeking help                ${c.dim("show this")}`,
      "",
      `  ${c.bold("privacy")}`,
      `    only counts and ratios leave your machine: tokens, sessions,`,
      `    session timing, file + line churn counts, model + tool-use +`,
      `    stop-reason + permission-mode breakdowns, hook event counts,`,
      `    response latency, hour-of-day histogram, public-marketplace`,
      `    skill + subagent breakdowns (user-specific names collapse to`,
      `    "other"). never: prompts, code, file paths, repo names,`,
      `    transcript content.`,
