...
const PollResponseSchema = v.union([
  ...
  v.object({
    ...
    token: v.string(),
    userId: v.string(),
    // Nullable: a user can complete OAuth before picking a handle (handle
    // setup happens on the web profile page). The CLI persists `null`
    // verbatim so post-publish output can detect "handle missing" and
    // prompt the user to set one, rather than silently falling back to
    // userId (which yielded confusing /u/<userId> URLs in v0.0.3).
...
export async function runLogin(opts: { open?: boolean } = {}): Promise<void> {
  ...
  while (Date.now() - startedAt < POLL_MAX_MS) {
    ...
    await writeConfig({
      ...
      // result.handle may be null when the user finished OAuth but
      // hasn't picked a handle yet — coerce to undefined so writeConfig
      // omits the field, matching the "field absent" semantics readers
      // expect (presence-checks at config.ts:tokenMatchesHost-style
      // gates downstream).
