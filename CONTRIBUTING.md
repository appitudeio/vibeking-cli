...
- **New scanners** — Cursor, Windsurf, Codex, Aider, Continue, anything else with a local session archive. Add a new module alongside [`src/scanner.ts`](./src/scanner.ts), wire it up to the upload payload. The `tool` discriminator on `DailyShardSchema` in [`src/redaction.ts`](./src/redaction.ts) draws from the `SUPPORTED_TOOLS` list defined in [`src/types.ts`](./src/types.ts) — both need a new entry.
