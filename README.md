...
1. **The scanner is right here.** [`apps/cli/src/scanner.ts`](./apps/cli/src/scanner.ts) is short enough to read in five minutes. It reads `~/.claude/projects/*/memory.jsonl`, pulls timestamps + model names + token counts, and that's it.
